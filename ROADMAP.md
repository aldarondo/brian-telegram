...
### Family features
- [ ] `[Code]` — **Recurring tasks / scheduler**: `config/schedules.json` (cron expression + family name + prompt text) driven by a `node-cron` loop that injects synthetic messages into the same queue as webhook traffic, so scheduled runs reuse session, rate limit, and reply paths. Examples: "Sun 18:00 summarize low grocery items for charles", "daily 07:00 calendar + weather rundown per user". Include unit tests for cron parsing and TZ handling.
