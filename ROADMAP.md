...
- [x] `[Code]` 2026-05-04 — **Group chat awareness**: detect Telegram group/supergroup chats, gate responses to @-mention or reply-to-bot, strip mention from prompt, route group threads to a `chat:<id>` session bucket so private DM context stays isolated, and tag history entries with `speaker: <familyName>`. New `TELEGRAM_BOT_USERNAME` env var; 22 new tests covering helpers + webhook routing (83 total passing).
...
## ✅ Completed
- [x] `[Code]` 2026-05-04 — Group chat awareness shipped: `chat.type` detection in `src/bot.js`, `isBotMentioned`/`isReplyToBot`/`stripBotMention`/`isGroupChat` helpers in `src/utils.js`, chat-keyed session bucket (`chat:<id>`) so private DMs stay isolated, speaker tagged on group history entries, new `TELEGRAM_BOT_USERNAME` env var, README "Group chats" section, 22 new tests (83 total passing).
